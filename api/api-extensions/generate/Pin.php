		...
		public static function process($getParams, $postParams){
			if(isset($postParams['phone_number'])){
				...
				elseif($userCount[0]['count'] > 1){
					throw new Exception('Conflicting Mobiles: Unable to generate PIN for '.$postParams['phone_number'].'. Contact support.', 409);
				}
				throw new Exception('Unknown number '.$postParams['phone_number'], 400);
			}
