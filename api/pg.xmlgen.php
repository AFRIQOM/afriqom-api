<?php
	...
	if(DATABASE_ENGINE != POSTGRESQL){
		exit("Database Engine is not PostgreSQL...");
	}
	...
	if(!file_exists(XML_DEFINITIONS_DIRECTORY."sessions.xml")){
		echo "-------------------------------------------------".$end_of_line;
		echo "session.xml not found in xml definitions".$end_of_line;
		echo "-------------------------------------------------".$end_of_line;
	}
  ...
  foreach($databaseServer as $databaseResource => $settings){
  	...
  	$rst = pg_query(Utils::getMySQLResource($databaseResource), "SELECT tablename FROM pg_catalog.pg_tables WHERE schemaname != 'pg_catalog' AND schemaname != 'information_schema';");

		echo "--- Generating XML: dbres=".$databaseResource." ---".$end_of_line;
		while($row = pg_fetch_assoc($rst)){
			$rst2 = pg_query(Utils::getMySQLResource($databaseResource), "SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = '".$row["tablename"]."';");

			$xml = "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			$xml.= "<".$row["tablename"].">\n";
			$xml.= "<table name=\"".$row["tablename"]."\">\n";

			while($row2 = pg_fetch_assoc($rst2)){
				if(ENABLE_API_FIELD_ENCRYPTION === TRUE){
					$char = substr($row2["column_name"], 0, 1);
					$data = base64_encode(gzdeflate($row2["column_name"]));
					$xml .= "<field api_field=\"".$char.substr(crc32($data), 0, 4)."\">".$row2["column_name"]."</field>\n";
				}
				else{
					$xml .= "<field api_field=\"".$row2["column_name"]."\">".$row2["column_name"]."</field>\n";
				}
			}
			$xml.= "</table>\n";
			$xml.= "<access mode=\"rw\" />\n";
			$xml.= "</".$row["tablename"].">\n";
			file_put_contents(XML_DEFINITIONS_DIRECTORY."/".$databaseResource.'.'.$row["tablename"].".xml",$xml);
			echo $databaseResource.'.'.$row["tablename"].".xml".$end_of_line;
		}
  }
	...
	if(ENABLE_PERMISSIONS === TRUE){
		...
		$adminRole = PGQueryBuilder::query(PGQueryBuilder::prepare('list', $xml, $param, null));
		...
		if(!$roleID){
			...
			$result = PGQueryBuilder::query(PGQueryBuilder::prepare('save', $xml, $param, null));		
	...
	function populateAdminRolePermissions($roleID, $databaseResource){
		...
		foreach (scandir(XML_DEFINITIONS_DIRECTORY) as $key => $value) {
			...
			if(sizeof($parts) == 3 && $parts[1]){
				...
				$role = PGQueryBuilder::query(PGQueryBuilder::prepare('list', $xml, $param, null));
				...
				$xml = simplexml_load_file(XML_DEFINITIONS_DIRECTORY.$databaseResource.'.permissions.xml');					
				PGQueryBuilder::query(PGQueryBuilder::prepare('save', $xml, $param, null));
